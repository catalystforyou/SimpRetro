...
def ASScore(r_mols, in_stock):
    return sum([1 if r_mol in in_stock else 0 for r_mol in r_mols.split('.')]) / len(r_mols.split('.'))
...
def merge(reactant_d):
    ...
    if len(reactants) > 50:
        return list(reactants)[:50], list(scores)[:50], list(templates)[:50]
    else:
    ...
    def __call__(self, inputs: list[Molecule], num_results: int = 50) -> list[BackwardPredictionList]:
        ...
        for x in inputs:
            ...
            for i , rule in enumerate(rule_k):
                out1 = []
                if rule.split('>>')[0].count('.') != x.count('.'):
                    continue
                # if len(reactants) >= 50:
                #     break
                try:
                    out1 = rdchiralRunText(rule, x_mapped, keep_mapnums=True)
                    ...
                    for reactant in out1:
                        canonical_r = canonical_smiles(reactant)
                        reactants.append(canonical_r)
                        scores.append(CDScore(Chem.MolFromSmiles(x_mapped), reactant.split('.')) + 2 * ASScore(canonical_r, self.instock_list) + 0.5 * RDScore(Chem.MolFromSmiles(x), [Chem.MolFromSmiles(r) for r in canonical_r.split('.')]) + 1 / len(out1))
                        # print(canonical_r)
                        # print(scores[-1])
                        # print(rule)
                        # print(CDScore(Chem.MolFromSmiles(x_mapped), reactant.split('.')), ASScore(canonical_r, self.instock_list), RDScore(Chem.MolFromSmiles(x), [Chem.MolFromSmiles(r) for r in canonical_r.split('.')]), 1 / len(out1))
